...
export function getFileCategory(
  ...
  // 优先使用 MIME 类型判断
  if (supportedMimeTypes.text.includes(mimeType)) return 'text';
  if (supportedMimeTypes.pdf.includes(mimeType)) return 'pdf';
  if (supportedMimeTypes.word.includes(mimeType)) return 'word';
  if (supportedMimeTypes.excel.includes(mimeType)) return 'excel';
  if (supportedMimeTypes.image.includes(mimeType)) return 'image';
